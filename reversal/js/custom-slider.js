...
// Slider
jQuery(document).ready(function () {
	window.slider= jQuery("#carousel2").parent().html();
    var carousel = jQuery("#carousel2").waterwheelCarousel({
        flankingItems: 2,
        movingToCenter: function ($item) {
            jQuery('#callback-output').prepend('movingToCenter: ' + $item.attr('id') + '<br/>');
        },
    ...
    jQuery('#s-prev2').bind('click', function () {
        carousel.prev();
        return false
    });

    jQuery('#s-next2').bind('click', function () {
        carousel.next();
        return false;
    });
...
if( jQuery('#carousel').length )    {
  ...
  window.RT = setTimeout(function(){
	var slider_parent=jQuery("#carousel").parent();
	jQuery("#carousel2").next().remove();
	jQuery("#carousel2").remove();
	slider_parent.append(window.slider);
